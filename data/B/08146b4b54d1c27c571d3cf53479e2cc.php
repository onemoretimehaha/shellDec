...
$showindexcount = $GLOBALS['db'] -> fetchcount("SELECT count(id) FROM ".tableex('moudle')." where cid='$cid' and ifshow<>'0';");
$neworder = $GLOBALS['db'] -> fetchcount("SELECT morder FROM ".tableex('moudle')." where cid='$cid' order by morder desc limit 1");
if(!$neworder) {
	$neworder=0;
}
$ifshowtemp='';
...
if(isset($_POST['minfo'])) {
	...
	foreach($_POST['minfo'] as $key=>$val) {
		...
		if(strlen($_POST['minfo'][$key])>0) {
			...
			if($showindexcount==0) {$ifshow=1;$showindexcount=1;}else {$ifshow=0;}
			if($mname=='title') {$ifshow=1;$showindexcount=1;$msetting=dbstr(json_encode(array('filterhtml'=>1,'adminsearch'=>1)));}
			if(strlen($mname)==0 && strlen($minfo)>1) {
				$mname=Pinyin(cut_str(trim($minfo),3));
			}
			$mname=trim($mname);
			if(strlen($mname)<1) {
				$msg.=$minfo.':字段名太短 ';
				...
				if(in_array($mname,$notallowfield)) {
					...
					$pagecount = $GLOBALS['db'] -> fetchcount("SELECT count(id) FROM ".tableex('moudle')." where mname='$mname' and cid='$cid'");
					if($pagecount>0) {
						$msg.=$minfo.':已存在该字段 ';
					}else {
						$strdefault='';
						...
						if($mkind==9) {$strdefault='now';}
						if($mkind==7) {$strdefault=1;}
						$query = $GLOBALS['db'] -> query("INSERT INTO `".tableex('moudle')."` (`mname` ,`minfo`,`mkind`,`morder`,`cid`,`ifshow`,`ifonly`,`strdefault`,`mcontent`,`ifshowtemp`,`hide`,`ifadmin`,`fid`,`msetting`,`ifcreated`) VALUES ('$mname','$minfo','$mkind','$morder','$cid','$ifshow','0','$strdefault','','$ifshowtemp','0','0','','$msetting','0');");
						if($query) {
						}else {
							$msg.=$minfo.':添加失败 ';
						}
